...
class MoodService {
  ...
  MoodService({String? assetsPath})
    ...
    print('⚡️ MoodService ініціалізовано. Шлях: $_assetsPath');
  }
  ...
  static String _resolveAssetsPath() {
    try {
      ...
      throw Exception('Директорію assets не знайдено за шляхами: $candidates');
    } catch (e) {
      print('❌ Помилка визначення шляху: $e');
  ...
  Future<MoodResponse> calculateMood({
    ...
    try {
      ...
      print('✅ Настрій для $climateZone: E:${energy.toStringAsFixed(2)} V:${valence.toStringAsFixed(2)}');
      return MoodResponse(energy: energy, valence: valence);

    } catch (e, stack) {
      print('❌ Критична помилка у calculateMood: $e');
      print(stack);
  ...
  Future<Map<String, dynamic>> _loadJson(String relativePath) async {
    ...
    if (!await file.exists()) {
      print('⚠️ Файл відсутній: $fullPath. Використовується порожній конфіг.');
      return {};
    }
  ...
  static String _getPrecipitationType(String desc, int code) {
    ...
    if (d.contains('дощ') || d.contains('rain')) return 'light_rain';
